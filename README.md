...
| Binary | twise | Generates a configuration for each combination of configuration options with the size of t. NOTE:While the same interactions as pairwise and featurewise are covered with t:2 and t:2, the number of configurations is not necessarily the same.| ```binary twise t:<int>``` | binary twise t:4 |
